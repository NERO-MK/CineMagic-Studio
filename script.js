...
function handleVoiceUpload(input) {
    if(input.files && input.files[0]) { selectedVoiceFile = input.files[0]; alert("Lip-Sync Sample Loaded. Cloned voice will follow original actor's lips."); }
}

// --- Note *: ACTION DISPATCHER ---
async function triggerAction(type) {
    document.getElementById('actionMenu').classList.add('hidden');
    if(!currentProjectId) { alert("Begin production first."); return; }
    
    alert(`🚀 Dispatching to ${type.toUpperCase()} via CineMagic Cloud Tunnel...`);
    // API logic to trigger Social Sync Pro or Telegram
}
...
async function handleSend() {
    ...
    userMsg.innerHTML = `<div class="bg-slate-900 text-white px-8 py-5 rounded-[2.5rem] text-sm max-w-[85%] shadow-xl font-medium text-left whitespace-pre-wrap">${input || "Initiating Multimodal Lip-Sync..."}</div>`;
    history.appendChild(userMsg);

    const thinking = document.createElement('div');
    thinking.className = "flex gap-6 items-start animate-in fade-in mb-8";
    thinking.innerHTML = `<div class="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-600 text-xs italic shrink-0 border border-blue-500/20 shadow-sm">⚡</div><p class="text-sm font-bold text-slate-400 uppercase tracking-widest animate-pulse mt-2.5 italic">Lead Producer is architecting 4K Master with Lip-Sync...</p>`;
    ...
    try {
        ...
        if (projects && projects.length > 0) {
            container.innerHTML = projects.map(p => `
                <div onclick="alert('Restore: ${p.title}')" class="flex flex-col gap-1 px-5 py-5 bg-slate-50 border border-slate-100 rounded-3xl cursor-pointer hover:border-primary transition-all text-left mb-3 shadow-sm group">
